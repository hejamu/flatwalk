...
class TestRun:
    ...
    def test_progress_callback_fires_once_per_check_with_owned_copies(self):
        """The callback should fire exactly once per ``n_check`` and the
        arrays in the snapshot must be copies (mutating them must not
        affect subsequent WL updates)."""
        sys = _tiny_system(5)
        cfg = WLConfig(bin_scheme=sys["scheme"], n_check=100, ln_f_final=1e-30)
        snapshots = []

        def cb(snap):
            # Verify shape and types
            assert snap.g.shape == (sys["scheme"].n_bins,)
            assert snap.H.shape == (sys["scheme"].n_bins,)
            assert snap.visited.shape == (sys["scheme"].n_bins,)
            assert isinstance(snap.t, int)
            assert isinstance(snap.ln_f, float)
            assert isinstance(snap.in_1overt, (bool, np.bool_))
            # Try to mutate — this must not affect the driver.
            snap.g[:] = -99.0
            snap.H[:] = 0
            snap.visited[:] = False
        ...
        # Exactly t // n_check checks fired
        assert len(snapshots) == 500 // 100
        # The most recent snapshot was clobbered; the result.g should NOT be -99
        assert not np.any(result.g == -99.0), "snapshot mutation leaked into driver state"
        # t values monotonically increasing in multiples of n_check
    ...
    def test_progress_callback_unused_when_none(self):
        ...
        sys = _tiny_system(5)
        cfg = WLConfig(bin_scheme=sys["scheme"], n_check=100, ln_f_final=1e-30)
        # Just confirm the run completes without error when callback is None.
        result = WLDriver(cfg).run(
            initial_state=sys["initial_state"],
            energy_fn=sys["energy_fn"],
            order_parameter_fn=sys["order_parameter_fn"],
